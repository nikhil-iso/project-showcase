...
const personalProjects: Project[] = [
  {
    ...
    details:
      "Designed as a modular base for future rocket avionics: separate driver modules for the IMU, barometer, and storage so new sensors can be added without touching the main loop.\n\nGround pressure is sampled and averaged at boot to set a baseline for altitude calculations. Telemetry is streamed as CSV over USB serial for live debugging and mirrored to SD for post-flight review. A piezo buzzer reports init status and sensor errors so failures are caught before launch.",
  },
  {
    ...
    details:
      "ESP32 firmware drives a high-side MOSFET via PWM to dim a 12V LED panel, with a DS3231 RTC as a fallback when Wi-Fi time sync is unavailable. Schedules are stored as JSON so multiple alarm profiles can coexist.\n\nA Python desktop utility lets you tune ramp profiles visually and push them to the device. KiCad schematic and a Fusion 360 enclosure round out the build into something repeatable.",
  },
  {
    ...
    details:
      "Uses the Arduino Micro's native USB HID stack so the device enumerates as a standard keyboard — no drivers required on any major OS. The 4×2 matrix is scanned with debounce in firmware, and macros are stored in EEPROM so profiles survive power cycles.\n\nThe enclosure was modeled in Fusion 360 with hot-swap sockets in mind, so switches can be changed without soldering. Total BOM was under $40.",
  },
  {
    ...
    details:
      "Pipeline is split into independent detection, tracking, and actuation stages so each can be swapped or tuned in isolation. Currently focused on getting reliable tracking of moving targets under varying lighting before integrating the motor control stage.\n\nLong-term goal is a small-scale demonstrator inspired by Carbon Robotics' laser weeder — pattern recognition driving a 2-axis aiming mechanism.",
  },
];
